  ...
  Future<void> _saveMember() async {
    if (_nameController.text.isEmpty || _emailController.text.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text("Fyll i namn och e-post!")));
      return;
    }

    if (_editingId == null && _passwordController.text.length < 6) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Lösenordet måste vara minst 6 tecken.")),
      );
      return;
    }
    ...
    try {
      if (_editingId == null) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("Medlem skapad säkert!")));
      } else {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("Medlem uppdaterad!")));
      }
      _resetForm();
    } on FirebaseAuthException catch (e) {
      String msg = "Ett fel uppstod vid skapandet av kontot.";
      if (e.code == 'email-already-in-use')
        msg = "Denna e-postadress används redan.";
      if (e.code == 'invalid-email')
        msg = "E-postadressen är felaktigt formaterad.";
      ScaffoldMessenger.of(
        context,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Något gick helt fel: $e"),
